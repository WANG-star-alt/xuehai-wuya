...
(function () {
  ...
  const branchInterface = createNode(
    'interface',
    '界面篇',
    '从"GUI 是什么"到"AI 时代的生成式界面"——10 章讲透人和机器之间那层看得见、摸得着的皮肤；第 5 章按"你要做什么"组织桌面 GUI 实战。',
    '第 1–4 章完结 · 第 5 章更新中',
    [
      ...
      createNode('gui-03', '第 3 章 · 布局与排版',
        ...
        [
          ...
          createNode('gui-03-3', '§ 3.3 · Flexbox', '一维弹性布局——主轴、交叉轴、伸缩与对齐。', '9578 字', [], { href: 'chapters/gui/03-3-flexbox.html', spec: 'pass'}),
          createNode('gui-03-4', '§ 3.4 · Grid 网格', '二维网格布局——真正的"报纸排版"能力。', '10051 字', [], { href: 'chapters/gui/03-4-grid.html', spec: 'pass'}),
          createNode('gui-03-5', '§ 3.5 · 约束布局', '原生 App 的另一套思路——AutoLayout / ConstraintLayout。', '9507 字', [], { href: 'chapters/gui/03-5-constraint.html', spec: 'pass'}),
          createNode('gui-03-6', '§ 3.6 · 响应式与自适应', '媒体查询、断点、容器查询——一套界面适配所有屏幕。', '9791 字', [], { href: 'chapters/gui/03-6-responsive.html', spec: 'pass'}),
        ],
      ...
      createNode('gui-04', '第 4 章 · 事件与交互',
        ...
        [
          ...
          createNode('gui-04-5', '§ 4.5 · 焦点与键盘导航', 'Tab 顺序、焦点陷阱、快捷键——不用鼠标也能用。', '9656 字', [], { href: 'chapters/gui/04-5-focus.html', spec: 'pass'}),
          createNode('gui-04-6', '§ 4.6 · 无障碍 A11y', 'ARIA、屏幕阅读器、对比度——让所有人都能用。', '9662 字', [], { href: 'chapters/gui/04-6-a11y.html', spec: 'pass'})
        ],
      ...
      createNode('gui-05', '第 5 章 · 桌面 GUI 工具箱',
        ...
        [
          ...
          createNode('gui-05-5', '§ 5.5 · 网页技术与原生路线：Electron / Tauri / Win32 / Avalonia', 'VS Code 的 Electron、体积只有其十分之一的 Tauri，兼看微软四代原生栈与 C# 跨平台双雄。', '待撰写', []),
          createNode('gui-05-6', '§ 5.6 · 打包与分发：让别人双击就能用', 'PyInstaller / Nuitka / 安装包 / 签名 / 自动更新——做完程序的最后一步。', '待撰写', [])
        ],
        { href: 'chapters/gui/05-1-choice-map.html' }),
  ...
  const branchElec = createNode(
    ...
    [
      ...
      createNode('elec-06', '第 6 章 · 工具箱与万用表',
        '三百块配齐一套家用工具，学会万用表的四个常用档位——后面排查跳闸、判断电器都要靠它，从"猜"升级成"测"。',
        '待撰写',
        [
          createNode('elec-06-1', '§ 6.1 · 新手工具清单', '螺丝刀、验电笔、剥线钳、胶布、头灯——够用就好。', '待撰写', []),
          createNode('elec-06-2', '§ 6.2 · 万用表面板怎么看', '档位、表笔插孔、量程，开机第一步别接错。', '待撰写', []),
          createNode('elec-06-3', '§ 6.3 · 测电压：交流与直流', '测插座 220 伏、测电池 1.5 伏，最常用两个操作。', '待撰写', []),
          createNode('elec-06-4', '§ 6.4 · 测通断与电阻', '蜂鸣档查断线、查保险、查开关是不是真的通。', '待撰写', []),
          createNode('elec-06-5', '§ 6.5 · 测电流与计量插座', '钳形表和功率计量插座，看清电器真实耗电。', '待撰写', []),
          createNode('elec-06-6', '§ 6.6 · 剥线、压接、接线端子', '不用电焊也能接得牢的三种正规接线做法。', '待撰写', []),
          createNode('elec-06-7', '§ 6.7 · 绝缘、防水与固定', '热缩管、防水接线盒、线卡——修完还得能用十年。', '待撰写', [])
        ]
      ),
      createNode('elec-07', '第 7 章 · 跳闸了怎么查',
        '家里最常见的"突然黑了"——用半拆法十分钟锁定回路，再找到那台惹事的电器。',
        '待撰写',
        [
          createNode('elec-07-1', '§ 7.1 · 跳闸的四种原因', '过载、短路、漏电、开关老化，先分清是哪一类。', '待撰写', []),
          createNode('elec-07-2', '§ 7.2 · 半拆法定位故障回路', '全关再逐个合闸，用排除法把范围压到一路。', '待撰写', []),
          createNode('elec-07-3', '§ 7.3 · 逐个拔插头找元凶', '回路内一台台断开再合闸，揪出真正漏电的设备。', '待撰写', []),
          createNode('elec-07-4', '§ 7.4 · 潮湿季节的漏保跳闸', '卫生间、阳台、热水器受潮漏电的典型现场。', '待撰写', []),
          createNode('elec-07-5', '§ 7.5 · 闸推不上去怎么办', '手柄半档、复位按钮、先复位再合闸，别硬掰。', '待撰写', []),
          createNode('elec-07-6', '§ 7.6 · 什么时候必须停手', '反复跳闸、闻到糊味、墙面发热——立刻叫电工。', '待撰写', [])
        ]
      ),
      createNode('elec-08', '第 8 章 · 电器不工作怎么判断',
        '一套对任何电器都适用的判断框架——先确认有没有电，再判断坏在哪一段，最后决定修还是换。',
        '待撰写',
        [
          createNode('elec-08-1', '§ 8.1 · 三问法排查框架', '有电吗、通不通、断在哪段——任何电器都能套。', '待撰写', []),
          createNode('elec-08-2', '§ 8.2 · 先测插座有没有电', '换插座试、验电笔、万用表，三种验证由简到准。', '待撰写', []),
          createNode('elec-08-3', '§ 8.3 · 插头与电源线故障', '内芯暗断、插头松动是最常见故障，也最好修。', '待撰写', []),
          createNode('elec-08-4', '§ 8.4 · 保险丝与热保护器', '微波炉、吹风机里的小保险，换对型号就复活。', '待撰写', []),
          createNode('elec-08-5', '§ 8.5 · 电机类：不转与嗡嗡响', '风扇、油烟机、水泵的卡死与启动电容老化。', '待撰写', []),
          createNode('elec-08-6', '§ 8.6 · 加热类：不热与忽冷忽热', '电热管、温控器、水壶底座触点的判断顺序。', '待撰写', []),
          createNode('elec-08-7', '§ 8.7 · 修还是换：算一笔账', '配件价、工时、剩余寿命、安全余量——四项打分。', '待撰写', [])
        ]
      ),
  ...
  const treeRoot = createNode(
    'root',
    '学海无涯',
    '把 AI、网络、界面、电学四大领域拆成看得见、点得动的认知地图——每篇独立、循序渐进。',
    '约 143 万字 · 4 篇 45 章 · 更新中',
